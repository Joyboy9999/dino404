  ...
  async function mint(wallet) {
    ...
    if(checkwl){
      try {
        ...
        const transaction = await ContractWithSigner.mintERC20({ value: tokenAmountInEther, gasLimit });
...
const PopupMinting = ({walletAddress, popupMinting, setPopupMinting }) => {
  ...
  const provider = new ethers.providers.Web3Provider(window.ethereum);
  const contract = new ethers.Contract(tokenAddress, tokenABI, provider);
  contract.whitelist(walletAddress)
